class CircuitSim {
    ...
    findSource() {
        this.source = window.components.find(
            (c) =>
                c.type === "battery" && !c.componentObject.getData("isInPanel")
        );
        return this.source;
    }
    ...
    generate_tree() {
        ...
        if (!this.validateBatteryWiring()) {
            console.error(
                "Battery wiring error: wires must be connected to opposite poles (start and end nodes)"
            );
            return;
        }

        const allPaths = [];
        this.collectAllPaths(
            this.source,
            this.source.end,
            new Set(),
            [this.source.id],
    ...
    validateBatteryWiring() {
        ...
        const startHasWire =
            this.source.start.wire !== null &&
            this.source.start.wire !== undefined;
        const endHasWire =
            this.source.end.wire !== null && this.source.end.wire !== undefined;
    ...
    collectAllPaths(currentComponent, currentNode, visited, path, allPaths) {
        ...
        const nextNode =
            currentNode === currentComponent.start
                ? currentComponent.end
                : currentComponent.start;
        const adjacentComponents = this.getAdjacentComponents(
            nextNode,
            currentComponent
        );

        const batteryInAdjacent = adjacentComponents.find(
            (comp) => comp.id === this.source.id
        );
        ...
        if (batteryInAdjacent && currentComponent.id !== this.source.id) {
            ...
            if (
                this.source.start.wire &&
                returningNode.wire === this.source.start.wire
            ) {
                batteryReturnNode = this.source.start;
            } else if (
                this.source.end.wire &&
                returningNode.wire === this.source.end.wire
            ) {
        ...
        const unvisitedComponents = adjacentComponents.filter(
            (comp) => !visited.has(comp.id)
        );
        ...
        for (const nextComponent of unvisitedComponents) {
            ...
            if (nextNode.wire && nextComponent.start.wire === nextNode.wire) {
                ...
                nextNode.wire &&
                nextComponent.end.wire === nextNode.wire
            ) {
                nextComponentNode = nextComponent.end;
            } else {
                nextComponentNode =
                    nextComponent.start === nextNode ||
                    (nextComponent.start.x === nextNode.x &&
                        nextComponent.start.y === nextNode.y)
                        ? nextComponent.start
                        : nextComponent.end;
            }
        ...
        for (const wireNode of node.wire.nodes) {
            if (wireNode.component) {
                if (wireNode.component.id !== excludeComponent.id) {
                    if (
                        wireNode === wireNode.component.start ||
                        wireNode === wireNode.component.end
                    ) {
                        if (
                            !adjacentComponents.find(
                                (c) => c.id === wireNode.component.id
                            )
                        ) {
    ...
    init() {
        this.generate_tree();
        this.running_sim = !this.running_sim;
        this.run_sim();
    }
        ...
        for (const path of this.paths) {
            ...
            for (let i = 1; i < path.length - 1; i++) {
                ...
                if (directionalTypes.includes(component.type)) {
                    ...
                    const prevComponent = window.components.find(
                        (c) => c.id === prevComponentId
                    );
                    if (!directionalTypes.includes(prevComponent.type)) {
                        break;
                    }
                    ...
                    if (prevComponent.type === "battery") {
                        if (
                            !this.negativeVoltage &&
                            prevComponent.start.wire &&
                            component.start.wire === prevComponent.start.wire
                        ) {
                            enteredFromStart = true;
                        } else if (
                            this.negativeVoltage &&
                            prevComponent.end.wire &&
                            component.start.wire === prevComponent.end.wire
                        ) {
                            enteredFromStart = true;
                        }
                    } else if (
                        (prevComponent.start.wire &&
                            component.start.wire ===
                                prevComponent.start.wire) ||
                        (prevComponent.end.wire &&
                            component.start.wire === prevComponent.end.wire)
                    ) {
    ...
    run_sim() {
        ...
        // Get battery and its clock speed
        const battery = this.source || this.findSource();
        if (!battery) {
            console.warn("No battery found for simulation");
            return;
        }

        // Get clock speed from battery values
        let clockSpeed = 1; // Default 1 Hz
        if (battery.values && battery.values.clockSpeed !== undefined) {
            clockSpeed =
                typeof battery.values.clockSpeed === "object"
                    ? battery.values.clockSpeed.value
                    : battery.values.clockSpeed;
        }

        // Run simulation at clock speed frequency (Hz)
        ...
        this.simIntervalId = setInterval(() => {
            if (this.running_sim) {
                ...
                // Stop the interval if simulation is no longer running
                clearInterval(this.simIntervalId);
                this.simIntervalId = null;
            }
        }, intervalMs);
        ...
        console.log(
            `Simulation running at ${clockSpeed} Hz (${intervalMs}ms interval)`
        );
    }
    ...
    checkNegativeVoltage() {
        if (
            !this.source ||
            !this.source.values ||
            this.source.values.voltage === undefined
        ) {
            this.negativeVoltage = false;
            return false;
        }
        ...
        const batteryVoltage =
            typeof this.source.values.voltage === "object"
        ...
        if (batteryVoltage < 0) {
            ...
            console.log(
                "⚠ Battery voltage is negative, paths will be reversed"
            );
            // Reverse all paths
            ...
            if (Array.isArray(item)) {
                ...
                if (
                    ...
                    const componentValues = {
                        ...
                        type: component.type,
                        resistance: this.extractValue(
                            component.values.resistance
                        ),
                        voltage: this.extractValue(
                            component.values.voltageDrop ||
    ...
    getValuesSummary() {
        ...
        return values
            .map((v) => {
                const r =
                    v.resistance !== undefined
                        ? `${v.resistance}Ω`
                        : "undefined";
                const volt =
                    v.voltage !== undefined ? `${v.voltage}V` : "undefined";
                const i =
                    v.current !== undefined ? `${v.current}A` : "undefined";
    ...
    createTreeFromPaths() {
        ...
        while (suffixLength < paths[0].length - prefixLength) {
            ...
            if (
                paths.every(
                    (path) =>
                        path[path.length - 1 - suffixLength] === componentId
                )
            ) {
                suffixLength++;
            } else {
        ...
        const processLevel = (items, connectionType) => {
            ...
            if (connectionType === "series") {
                ...
                if (allSeriesComponents.length > 1) {
                    ...
                    allSeriesComponents.forEach((targetId, index) => {
                        const otherIds = allSeriesComponents.filter(
                            (id, i) => i !== index
                        );
                        ...
                        this.equationList.push({
                            ...
                            solve: (values) => {
                                ...
                                for (const otherId of otherIds) {
                                    const otherComp = this.nodeValues.find(
                                        (c) => c.id === otherId
                                    );
                                    if (
                                        otherComp &&
                                        otherComp.current !== undefined
                                    ) {
            ...
            if (connectionType === "parallel" && componentIds.length > 0) {
                ...
                if (branchRepresentatives.length > 1) {
                    ...
                    branchRepresentatives.forEach((targetId, index) => {
                        const otherIds = branchRepresentatives.filter(
                            (id, i) => i !== index
                        );

                        this.equationList.push({
                            type: "kcl_parallel",
                            equation: `${targetId}.current = I_battery - ${otherIds
                                .map((id) => `${id}.current`)
                                .join(" - ")}`,
                            solve: (values) => {
                                // Need battery current and all other branch currents
                                const battery = this.nodeValues.find(
                                    (c) => c.type === "battery"
                                );
            ...
            if (connectionType === "parallel" && componentIds.length > 1) {
                ...
                componentIds.forEach((targetId, index) => {
                    const otherIds = componentIds.filter(
                        (id, i) => i !== index
                    );
    ...
    generateParallelCurrentSumEquations() {
        ...
        for (
            let i = 0;
            i < Math.min(...this.activePaths.map((p) => p.length));
            i++
        ) {
        ...
        for (
            ...
            const firstPathComp =
                this.activePaths[0][this.activePaths[0].length - i];
            if (
                this.activePaths.every(
                    (path) => path[path.length - i] === firstPathComp
                )
            ) {
        ...
        const afterComp = this.nodeValues.find(
            (c) => c.id === componentAfterParallel
        );
        ...
        for (const path of this.activePaths) {
            if (divergeIndex < path.length) {
                ...
                const branchComp = this.nodeValues.find(
                    (c) => c.id === branchCompId
                );
        ...
        this.equationList.push({
            ...
            equation: `${componentAfterParallel}.current = ${branchComponents
                .map((id) => `${id}.current`)
                .join(" + ")}`,
            solve: (values) => {
        ...
        console.log(
            `Generated parallel sum equation: ${afterComp.name}.current = sum of ${branchComponents.length} branches`
        );
    }
    ...
    solvePool() {
        ...
        while (changesMade && iterations < maxIterations) {
            ...
            for (
                let eqIndex = 0;
                eqIndex < this.equationList.length;
                eqIndex++
            ) {
                ...
                if (result) {
                    ...
                    console.log(
                        `✓ Solved ${eq.type} (eq#${eqIndex}) for ${
                            comp.name
                        }: ${eq.solves} = ${result[eq.solves]}`
                    );
            ...
            const pathVoltageChanges =
                this.calculateVoltagesFromPaths(definedVariables);
        ...
        console.log(
            ...
            Array.from(definedVariables).sort()
        );
        ...
        const stillUndefined = allPossibleVars.filter(
    ...
    applyKVLSeries(definedVariables) {
        ...
        const seriesEquations = this.equationList.filter(
            (eq) => eq.type === "kvl_series"
        );
        ...
        for (const eq of seriesEquations) {
            ...
            const components = eq.components
                .map((id) => this.nodeValues.find((c) => c.id === id))
            ...
            const parallelSiblingVoltage =
                this.getParallelSiblingVoltageFromPaths(eq.components);
            ...
            if (referenceVoltage !== null) {
                const knownVoltages = components.filter(
                    (c) => c.voltage !== undefined
                );
                const unknownVoltages = components.filter(
                    (c) => c.voltage === undefined
                );
                ...
                if (unknownVoltages.length === 1 && knownVoltages.length > 0) {
                    ...
                    for (const knownComp of knownVoltages) {
                        ...
                        if (referenceSource !== "parallel sibling") {
                            const nestedVoltage =
                                this.getEffectiveVoltageFromPaths(knownComp.id);
                            sumKnown +=
                                nestedVoltage !== null
                                    ? nestedVoltage
                                    : knownComp.voltage;
                    ...
                    definedVariables.add(varName);
                    console.log(
                        `✓ KVL Series (eq#${eqIndex}): ${unknownVoltages[0].name}.voltage = ${unknownVoltages[0].voltage}V (${referenceVoltage}V from ${referenceSource} - ${sumKnown}V)`
                    );
                    console.log(`  Equation: ${eq.equation}`);
                    console.log(`  Defined: ${varName}`);
                    changesMade = true;
    ...
    calculateVoltagesFromPaths(definedVariables) {
        ...
        if (this.activePaths.length > 1) {
            for (
                ...
                const firstPathComp = this.activePaths[0][i];
                if (
                    this.activePaths.every((path) => path[i] === firstPathComp)
                ) {
                    divergeIndex = i + 1;
                } else {
                    break;
        ...
        console.log(
            `[Path Voltage Calc] Diverge at index ${divergeIndex}, converge at index ${convergeIndex}`
        );
        ...
        for (const path of this.activePaths) {
            ...
            for (let i = 1; i < path.length - 1; i++) {
                ...
                const isInParallelSection =
                    divergeIndex < convergeIndex &&
                    i >= divergeIndex &&
                    i < convergeIndex;
            ...
            if (componentsToCalculate.length === 1) {
                const { comp, varName, isInParallelSection } =
                    componentsToCalculate[0];
                ...
                if (isInParallelSection && this.activePaths.length > 1) {
                    for (const siblingPath of this.activePaths) {
                        ...
                        for (
                            let i = divergeIndex;
                            i < convergeIndex && i < siblingPath.length;
                            i++
                        ) {
                            const sibCompId = siblingPath[i];
                            const sibComp = this.nodeValues.find(
                                (c) => c.id === sibCompId
                            );
                        ...
                        if (hasAllVoltages) {
                            ...
                            console.log(
                                `[Path Voltage Calc] Using sibling voltage ${siblingVoltage}V as reference for ${comp.name}`
                            );
                            break;
                        }
                    }
                ...
                const calculatedVoltage =
                    referenceVoltage - totalCalculatedVoltage;
                comp.voltage = calculatedVoltage;
                definedVariables.add(varName);
                console.log(
                    `✓ Path Voltage Calc: ${comp.name}.voltage = ${calculatedVoltage}V (${referenceVoltage}V - ${totalCalculatedVoltage}V)`
                );
                console.log(`  Defined: ${varName}`);
                changesMade = true;
            }
    ...
    getEffectiveVoltageFromPaths(componentId) {
        ...
        const pathsWithComponent = this.activePaths.filter((path) =>
        ...
        if (pathsWithComponent.length > 1) {
            ...
            for (
                let i = 0;
                i < Math.min(...pathsWithComponent.map((p) => p.length));
                i++
            ) {
                const firstPathComp = pathsWithComponent[0][i];
                if (
                    pathsWithComponent.every(
                        (path) => path[i] === firstPathComp
                    )
                ) {
            ...
            for (
                ...
                const firstPathComp =
                    pathsWithComponent[0][pathsWithComponent[0].length - i];
                if (
                    pathsWithComponent.every(
                        (path) => path[path.length - i] === firstPathComp
                    )
    ...
    getParallelSiblingVoltageFromPaths(seriesComponentIds) {
        ...
        const firstCompIndex = pathWithComponents.indexOf(
            seriesComponentIds[0]
        );
    ...
    crossValidate() {
        ...
        this.nodeValues.forEach((comp) => {
            ...
            if (
                comp.voltage !== undefined &&
                comp.current !== undefined &&
                comp.resistance !== undefined
            ) {
                ...
                if (error > tolerance) {
                    console.warn(
                        `⚠ ${comp.name}: Ohm's law mismatch! V=${comp.voltage}V but I*R=${calculatedV}V (error: ${error}V)`
                    );
                    hasErrors = true;
                } else {
            ...
            if (
                ...
                if (error > tolerance) {
                    console.warn(
                        `⚠ ${comp.name}: Power mismatch! P=${comp.power}W but V*I=${calculatedP}W (error: ${error}W)`
                    );
                    hasErrors = true;
                } else {
    ...
    updateComponents() {
        ...
        this.nodeValues.forEach((nodeValue) => {
            ...
            // Update resistance
            if (
                nodeValue.resistance !== undefined &&
                component.type !== "battery"
            ) {
            ...
            if (nodeValue.voltage !== undefined) {
                ...
                const voltageValue = this.negativeVoltage
                    ? -nodeValue.voltage
                    : nodeValue.voltage;
            ...
            console.log(
                `✓ Updated ${component.values.name} [${updatedFields.join(
                    ", "
                )}]:`,
                {
                    R:
                        nodeValue.resistance !== undefined
                            ? `${nodeValue.resistance}Ω`
                            : "-",
                    V:
                        nodeValue.voltage !== undefined
                            ? `${nodeValue.voltage}V`
                            : "-",
                    I:
                        nodeValue.current !== undefined
                            ? `${nodeValue.current}A`
                            : "-",
                    P:
                        nodeValue.power !== undefined
                            ? `${nodeValue.power}W`
                            : "-",
                }
            );
